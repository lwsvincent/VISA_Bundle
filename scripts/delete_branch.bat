...
REM Usage: delete_branch.bat <branch_name> [-silent]

if "%1"=="" (
    echo Usage: delete_branch.bat ^<branch_name^> [-silent]
    echo Example: delete_branch.bat feature/old-feature
    echo          delete_branch.bat feature/old-feature -silent
    echo.
    echo This script will delete both local and remote branches.
    echo Use -silent to skip confirmation prompt.
    exit /b 1
)
...
if /i "%2"=="-silent" set SILENT_MODE=true
...
if /i "%SILENT_MODE%"=="true" (
    echo Silent mode enabled - proceeding with deletion without confirmation...
